...
export class AddUserToBarbers1751085900000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "barbers" ALTER COLUMN "userId" SET NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE "barbers" ADD CONSTRAINT "UQ_e06dd8ec5b263904f2b2c32e0dd" UNIQUE ("userId")`,
    );
    await queryRunner.query(
      `ALTER TABLE "barbers" ADD CONSTRAINT "FK_e06dd8ec5b263904f2b2c32e0dd" FOREIGN KEY ("userId") REFERENCES "users"("id") ON DELETE NO ACTION ON UPDATE NO ACTION`,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    // 1. Buscar userIds dos barbeiros para deletar os usuários correspondentes
    const barbers = await queryRunner.query(
      `SELECT "userId" FROM "barbers" WHERE "userId" IS NOT NULL`,
    );

    // 2. Remover constraints
    await queryRunner.query(
      `ALTER TABLE "barbers" DROP CONSTRAINT "FK_e06dd8ec5b263904f2b2c32e0dd"`,
    );
    await queryRunner.query(
      `ALTER TABLE "barbers" DROP CONSTRAINT "UQ_e06dd8ec5b263904f2b2c32e0dd"`,
    );
